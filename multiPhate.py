...
# if 'False', the raw Blast and Hmm outputs will be saved in the PipelineOutput folder
CLEAN_RAW_DATA_DEFAULT = 'True'
...
# default filename where gene sequences are written, per genome's PipelineOutput/
GENE_FILE = 'gene.fnt'
# default filename where protein sequences are written, per genome's PipelineOutput/
PROTEIN_FILE = 'protein.faa'
CGP_CODE_NAME = 'cgp_driver.py'        # top-level, driver program for running CompareGeneProfiles pipeline
# absolute path of top-level, driver for CompareGeneProfiles pipeline
CGP_CODE = CGP_DIR_DEFAULT + CGP_CODE_NAME
GENOMICS_CODE_NAME = 'genomics_driver.py'   # top-level, driver program for running Genomics analysis
# absolute path of top-level, drive for Genomics analysis
GENOMICS_CODE = GENOMICS_DIR_DEFAULT + GENOMICS_CODE_NAME
...
# default; sets a lower limit based on value at which a structure model can provide information
MIN_BLASTP_IDENTITY = 5
# default; sets a lower limit based on value at which a structure model can provide information
MIN_BLASTN_IDENTITY = 5
...
os.environ["PHATE_NCBI_VIRUS_GENOME_BLAST_HOME"] = os.environ["PHATE_NCBI_VIRUS_BASE_DIR"] + \
    "Virus_Genome/" + "viral.genomic.fna"
os.environ["PHATE_NCBI_VIRUS_PROTEIN_BLAST_HOME"] = os.environ["PHATE_NCBI_VIRUS_BASE_DIR"] + \
    "Virus_Protein/" + "viral.protein.faa"
...
os.environ["PHATE_PHANTOME_BLAST_HOME"] = os.environ["PHATE_PHANTOME_BASE_DIR"] + \
    "Phantome_Phage_genes.faa"
os.environ["PHATE_PHAGE_ENZYME_BASE_DIR"] = DATABASE_DIR_DEFAULT + \
    "PhageEnzyme/"  # not yet in service
...
# HMM
os.environ["PHATE_NCBI_VIRUS_PROTEIN_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + \
    "NCBI/Virus_Protein/"  # not yet in service
os.environ["PHATE_NCBI_VIRUS_PROTEIN_HMM_HOME"] = os.environ["PHATE_NCBI_VIRUS_PROTEIN_HMM_BASE_DIR"] + "unknown"
os.environ["PHATE_NCBI_VIRUS_GENOME_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + \
    "NCBI/Virus_Genome/"  # not yet in service
os.environ["PHATE_NCBI_VIRUS_GENOME_HMM_HOME"] = os.environ["PHATE_NCBI_VIRUS_GENOME_HMM_BASE_DIR"] + "unknown"
os.environ["PHATE_REFSEQ_PROTEIN_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + \
    "Refseq/Protein_hmm/"  # not yet in service
os.environ["PHATE_REFSEQ_PROTEIN_HMM_HOME"] = os.environ["PHATE_REFSEQ_PROTEIN_HMM_BASE_DIR"] + "unknown"
os.environ["PHATE_REFSEQ_GENE_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + \
    "Refseq/Gene_hmm/"  # not yet in service
...
os.environ["PHATE_PHANTOME_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + \
    "PHANTOME_hmm/"  # not yet in service
os.environ["PHATE_PHANTOME_HMM_HOME"] = os.environ["PHATE_PHANTOME_HMM_BASE_DIR"] + \
    "unknown"  # not yet in service
os.environ["PHATE_PHAGE_ENZYME_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + \
    "phageEnzyme_hmm/"  # not yet in service
os.environ["PHATE_PHAGE_ENZYME_HMM_HOME"] = os.environ["PHATE_PHAGE_ENZYME_HMM_BASE_DIR"] + \
    "unknown"  # not yet in service
os.environ["PHATE_KEGG_VIRUS_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + \
    "KEGG_hmm/"  # not yet in service
...
os.environ["PHATE_PFAM_HMM_HOME"] = os.environ["PHATE_PFAM_HMM_BASE_DIR"] + \
    "unknown"  # not yet in service
os.environ["PHATE_SMART_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + "SMART_hmm/"  # not yet in service
os.environ["PHATE_SMART_HMM_HOME"] = os.environ["PHATE_SMART_HMM_BASE_DIR"] + \
    "unknown"  # not yet in service
os.environ["PHATE_SWISSPROT_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + "Swissprot_hmm/"
os.environ["PHATE_SWISSPROT_HMM_HOME"] = os.environ["PHATE_SWISSPROT_HMM_BASE_DIR"] + \
    "unknown"  # not yet in service
os.environ["PHATE_UNIPROT_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + \
    "UNIPROT_hmm/"  # not yet in service
os.environ["PHATE_UNIPROT_HMM_HOME"] = os.environ["PHATE_UNIPROT_HMM_BASE_DIR"] + \
    "unknown"  # not yet in service
os.environ["PHATE_NR_HMM_BASE_DIR"] = DATABASE_DIR_DEFAULT + "NR_hmm/"  # not yet in service
os.environ["PHATE_NR_HMM_HOME"] = os.environ["PHATE_NR_HMM_BASE_DIR"] + \
    "unknown"  # not yet in service
...
# global, if installed via conda; use blast+, not legacy blast
...
# *** This is the hit count actually used in phate_blast
os.environ["PHATE_HIT_COUNT_MAX"] = str(HIT_COUNT_MAX)
...
# Print warnings and errors to standard out
os.environ["PHATE_PHATE_WARNINGS"] = PHATE_WARNINGS_DEFAULT
# Print helpful messages (may be verbose)
os.environ["PHATE_PHATE_MESSAGES"] = PHATE_MESSAGES_DEFAULT
# Print each step in processing a genome
os.environ["PHATE_PHATE_PROGRESS"] = PHATE_PROGRESS_DEFAULT
# Capture standard errors to files on linux/mac machine
os.environ["PHATE_PHATE_ERR"] = PHATE_ERR
# Capture standard errors to files on linux/mac machine
os.environ["PHATE_PHATE_OUT"] = PHATE_OUT
...
HELP_STRING = """This code, """ + CODE + """, runs the phage annotation pipeline (phate_runPipeline.py) over multiple genomes. The configuration file input to this code specifies a list of genomes to be processed and the parameters for pipeline execution. The pipeline performs 1) gene calling by 4 gene callers (PHANOTATE, GeneMarkS, Glimmer3, and Prodigal), followed by identification of closest phage genome by means of blast against an NCBI-phage database, and sequence-based functional annotation by means of blastp against several peptide databases (NR, NCBI virus protein, KEGG-virus, Phantome, pVOGs, Swissprot, Refseq protein), and HMM search against the pVOG database. \nType: python """ + \
    CODE + """ usage - for more information about constructing the command line.\nType: python """ + CODE + \
    """ input - for more information about how this code can be run, or consult the README file for extensive information about installation and code usage.\n"""

INPUT_STRING = """The input files and other parameters for running this code are specified in a configuration file, which is provided as the only input parameter. See sample configuration file (""" + \
    SAMPLE_CONFIG_FILE + """) for details on how to customize your configuration file. Copy that file and then modify accordingly.\n"""
...
# captures the root name of the fasta file (e.g., takes 'P2' from P2.fasta)
p_root = re.compile("([\w\d_-]+)\.fasta")
...
# *** For now; but should distinguish between blastn/blastp
...
p_customProteinDBname = re.compile(
    "custom_protein_blast_database_name='(.*)'")  # not yet in service
...
p_customProteinDBpath = re.compile(
    "custom_protein_blast_database_path='(.*)'")  # not yet in service
...
# not yet in service (will this be combined with swissprot?)
...
for cLine in cLines:
    ...
    # ? is there an hmm profiles db for kegg? build it?
    ...
    # will this be combined w/Swissprot?
    ...
    if (match_comment or match_blank):
        ...
        if value != '':
            # be sure that name of subdir ends in exactly one '/' (user might omit the slash)
            value = value.rstrip('/')
            value = value + '/'
        ...
        if genomeDataItems != DATA_ITEMS_NUM:
            if not HPC:
                LOG.write("%s%s%s%s%s%s\n" % ("WARNING: check config file for possible incorrect data items: ",
                                              genomeDataItems, " for genome ", nextGenomeData["genomeName"], ' ', nextGenomeData["genomeNumber"]))
        genomeList.append(nextGenomeData)
        if not HPC:
    ...
    ##### Other processing #####
        ...
        if value.lower() == 'yes' or value.lower() == 'true' or value.lower() == 'on':
            ...
            if PHATE_WARNINGS == 'True':
                print(
                    "multiPhate says, WARNING: Invalid string following translate_only parameter in config file:", value)
            if not HPC:
                LOGFILE.write("%s%s\n" % (
    ...
    # Blast database locations
        ...
        if match_ncbiVirusProteinDBpath.group(1) != '':
            os.environ["PHATE_NCBI_VIRUS_PROTEIN_BLAST_HOME"] = match_ncbiVirusProteinDBpath.group(
                1)
        ...
        if match_phantomeDBpath.group(1) != '':
            ...
            os.environ["PHATE_PHANTOME_BASE_DIR"] = os.path.dirname(
                match_phantomeDBpath.group(1)) + '/'
        ...
        if match_keggVirusDBpath.group(1) != '':
            ...
            os.environ["PHATE_KEGG_VIRUS_BASE_DIR"] = os.path.dirname(
                match_keggVirusDBpath.group(1)) + '/'
    ...
    # Hmm database locations
        ...
        if match_ncbiVirusProteinHmmDBpath.group(1) != '':
            os.environ["PHATE_NCBI_VIRUS_PROTEIN_HMM_HOME"] = match_ncbiVirusProteinHmmDBpath.group(
                1)
...
if not HPC:  # Skip logging if running in high-throughput, else multiPhate.log files will clash
    ...
    if customHmm:
        LOG.write("%s%s\n" % ("   Custom hmm database is located in ",
...
for genome in genomeList:
    if not HPC:
        LOG.write("%s%c%s%c%s%c%s%c%s%c%s\n" % (genome["genomeNumber"], ' ', genome["genomeName"], ' ',
                                                genome["genomeType"], ' ', genome["genomeSpecies"], ' ', genome["genomeFile"], ' ', genome["outputSubdir"]))
...
for genome in genomeList:
    ...
    if match_root:
        ...
        if PHATE_WARNINGS:
            print("multiPhate says, WARNING: Fasta filename not recognized for genome",
                  genome["genomeName"])
            print("  Expected fasta filename extension: .fasta")
        if not HPC:
            LOG.write("%s%s%s\n" % ("WARNING: problem with fasta file: ",
                                    genome["genomeFile"], " This genome not processed!"))
        continue
...
if runCGP and not translateOnly and (len(genomeList) > 1):
    ...
    if runCGP:
        ...
        if CLEAN_PREVIOUS_CGP_RESULTS:
            ...
            if PHATE_WARNINGS:
                print(
                    "multiPhate says, CAUTION: Not removing previous CGP Results_ directories--data files will accumulate!")
