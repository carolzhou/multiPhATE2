...
class comparison(object):
        ...
        for directory in dirList:
            ...
            for cLine in cLines:
                ...
                if match_fileLine:
                    ...
                    print("TESTING: genomeFasta is",genomeFasta)
                    match_genomeName = re.search(p_genomeName,genomeFasta)
                    if match_genomeName:
                        genomeName = match_genomeName.group(1)
                    else:
                        print("TESTING: did not match genomeName")
                    #(genome,extension) = genomeFasta.split('.')
                    print("TESTING: genome is:",genome)
    ...
    def findGenomes(self,directory):
        ...
        for lLine in lLines:
            ...
            if match_genome1:
                ...
                match_genomeName = re.search(p_genomeName,genome1fasta)
                if match_genomeName:
                    genome1 = match_genomeName.group(1)
                else:
                    print("TESTING: did not match genomeName")
                print("TESTING: genome1 is",genome1)
                #(genome1,extension) = genome1fasta.split('.') 
            if match_genome2:
                (preamble,genomePath) = lLine.split(': ')
                genome2fasta = os.path.basename(genomePath)
                match_genomeName = re.search(p_genomeName,genome2fasta)
                if match_genomeName:
                    genome2 = match_genomeName.group(1)
                else:
                    print("TESTING: did not match genomeName")
                print("TESTING: genome2 is",genome2)
        ...
        for pLine in pLines:
            ...
            if match_genome:
                ...
                if genomePathString:
                    ...
                    match_genomeName = re.search(p_genomeName,genomefasta)
                    if match_genomeName:
                        genome = match_genomeName.group(1)
                    else:
                        print("TESTING: did not match genomeName")
    ...
    def addHit2genome(self,dataArgs): 
        ...
        if GENE:
            if dataArgs["gene1"] != "":
                print("TESTING: searching for genome object named:",dataArgs["genome1"])
                genome1_obj = self.findGenomeObject(dataArgs["genome1"])
                print("TESTING: this is what I found: ",genome1_obj)
    ...
    def findGenomeObject(self,genomeName):
        FOUND_GENOME = False
        print("TESTING: self.genomeList is:")
        for genomeObject in self.genomeList:
            print("  name:", genomeObject.name)
