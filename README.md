...
* Issue:  Some of the blast processes in the CompareGeneProfiles (CGP) module fail, yielding empty blast .out files in the Results_nnnnnn_nnnnnn directories. Solution: Turn off parallel processing in the CompareGeneProfiles module (ie, set cgp_threads='0' in your configuration file). We have discovered an issue with a third-party code, #makeblastdb. CompareGeneProfiles runs makeblastdb for each genome's gene and protein fasta files (in each genome subdirectory). When CompareGeneProfiles is run in parallel (using one process for each genomeXgenome comparison), makeblastdb intermittently fails to produce one or more of the database files, typically the index file (.nin or .pin), or other supporting files (e.g., .nsq, .psq, .phr). You can check whether these files were created by listinging the contents of a given genome's PhATE output ($ ls -l PipelineOutput/myGenomeSubdir/). For each myGenome_cgp_gene.fnt and myGenome_cgp_protein.faa file there should be 7 blast databases files (eg, .pdb, .phr, .pin, .pot, .psq, .ptf, .pto). If any one of these files is missing, then blast+ will fail, and the CGP_RESULTS/Results_nnnnnn_nnnnnn/ directories will show blast output files with 0 content). An error message will be generated by blast, depending on which missing file is encountered. For example, you might see an error message stating that no index file was found, or blast might throw a C++ null-pointer exception. We are looking into how to manage the use of makeblastdb when using parallel processing in multiPhATE2. For now, set cgp_threads='0' if you encounter this problem. 
...
2) The developers have detected an issue with makeblastdb. When running the code in parallel, makeblastdb occasionally fails to produce all of the necessary blast database files, which ultimately causes blast to fail. More details can be found in a [TROUBLESHOOTING](#makeblastdb) issue above. 
