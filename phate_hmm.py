...
class multiHMM(object):
    ...
    def __init__(self):
        ...
        self.KEGG_VIRUS_HMM           = False     # ditto
        self.REFSEQ_PROTEIN_HMM       = False     # ditto
    ...
    def setHmmParameters(self,paramset):
        if isinstance(paramset,dict):
            ...
            if 'ncbiVirusProteinHmm' in list(paramset.keys()):
                self.NCBI_VIRUS_PROTEIN_HMM = paramset["ncbiVirusProteinHmm"]
            if 'nrHmm' in list(paramset.keys()):
                self.NR_HMM = paramset["nrHmm"]
            if 'keggVirusHmm' in list(paramset.keys()):
                self.KEGG_VIRUS_HMM = paramset["keggVirusHmm"]
            if 'refseqProteinHmm' in list(paramset.keys()):
                self.REFSEQ_PROTEIN_HMM = paramset["refseqProteinHmm"]
            if 'refseqGeneHmm' in list(paramset.keys()):
                self.REFSEQ_GENE_HMM = paramset["refseqGeneHmm"]
    ...
    def setHmmProgram(self,hmmProgram): #*** Only jackhmmer is currently in service
        if(hmmProgram.lower() == 'hmmalign'):
            self.hmmProgram = 'hmmalign'
        elif(hmmProgram.lower() == 'hmmbuild'):
            self.hmmProgram = 'hmmbuild'
        elif(hmmProgram.lower() == 'jackhmmer'):
            self.hmmProgram = 'jackhmmer'
        elif(hmmProgram.lower() == 'phmmer'):
            self.hmmProgram = 'phmmer'
        elif(hmmProgram.lower() == 'hmmsearch'):
            self.hmmProgram = 'hmmsearch'
        elif(hmmProgram.lower() == 'hmmscan'):
            self.hmmProgram = 'hmmscan'
        else:
            if PHATE_WARNINGS == 'True':
                print("WARNING in hmm module:  Unrecognized HMM program:", hmmProgram) 
    ...
    def hmm1fasta(self,fasta,outfile,database,dbName): # fasta is a phate_fastaSequence.fasta object
        ...
        if self.hmmProgram == 'hmmalign':
            command = ''
        elif self.hmmProgram == 'hmmbuild':
            command = ''
        elif self.hmmProgram == 'jackhmmer':
            command = HMM_HOME + "jackhmmer --tblout " + seqOutfile + ' --domtblout ' + domOutfile + ' ' + fastaFile + ' ' + database
        elif self.hmmProgram == 'phmmer': # Recall: you cannot specify evalue, score, or bias, but can filter afterward
            command = HMM_HOME + "phmmer " + fastaFile + ' ' + database + ' > ' + outfile
        elif self.hmmProgram == 'hmmsearch':
            command = ''
        elif self.hmmProgram == 'hmmscan':
            command = ''
    ...
    def runHmm(self,fastaSet,dbType="protein"): # fastaSet is a phate_fastaSequence.multiFasta object
        ...
        if dbType.lower() == "protein" or dbType.lower == "aa" or dbType.lower == "prot" or dbType.lower == "peptide":
            ...
            if PHATE_WARNINGS == 'True':
                print("WARNING in hmm module: unrecognized database type in runBlast:", dbType)
            return
        ...
        if GENE:
            if self.REFSEQ_GENE_HMM:
                database = REFSEQ_GENE_BLAST_HOME # same database as blast uses
                dbName   = 'refseqGene'
                count = 0
        ...
        if PROTEIN:
            ...
            if self.NR_HMM:  
                database = NR_BLAST_HOME # same database as blast uses
                dbName   = 'nr'
                count = 0
                if PHATE_PROGRESS == 'True':
            ...
            if self.REFSEQ_PROTEIN_HMM:
                database = REFSEQ_PROTEIN_BLAST_HOME # same database as blast uses
            ...
            if self.PHANTOME_HMM:
                database = PHANTOME_BLAST_HOME # same database as blast uses
                dbName   = 'phantome'
                count = 0 
                if PHATE_PROGRESS == 'True':
            ...
            if self.KEGG_VIRUS_HMM: 
                database = KEGG_VIRUS_BLAST_HOME # same database as blast uses
                dbName   = 'kegg'
                count = 0
                if PHATE_PROGRESS == 'True':
