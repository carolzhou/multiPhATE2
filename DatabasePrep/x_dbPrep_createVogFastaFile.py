...
CODE_BASE = "dbPrep_createVogFastaFile"

# Paths to files; when running on mpath

VOG_BASE_DIR = ""
try:
    VOG_BASE_DIR = sys.argv[1] 
    if not os.path.isdir(VOG_BASE_DIR):
        print ("ERROR: dbPrep_createVogFastaFile says, No such directory: ",VOG_BASE_DIR)
        exit(0)
except:
    print ("ERROR: dbPrep_createVogFastaFile says, Input the VOG base directory")
    exit(0)

# Files

VOG_FASTA_MAP         = VOG_BASE_DIR + "vog.members.tsv"
GENE_FNT              = VOG_BASE_DIR + "vog.genes.all.fa"
PROTEIN_FAA           = VOG_BASE_DIR + "vog.proteins.all.fa"
VOG_TAGGED_GENES      = VOG_BASE_DIR + "VOGs.fnt"
VOG_TAGGED_PROTEINS   = VOG_BASE_DIR + "VOGs.faa"
LOG                   = VOG_BASE_DIR + CODE_BASE + ".log"
PVOG_LIB_DIR          = VOG_BASE_DIR + "Allvogtables/"
