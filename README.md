# multiPhATE v.2.0
/MultiPhATE/ - multiPhATE2 (beta version) - This code is has been thoroughly tested in the hands of the developers, but is considered beta until it passes peer review. Feel free to download and test drive the current code (select the green "Code" button).  Thank you for your interest in multiPhATE2. For a production version of multiPhATE, use the code at https://github.com/carolzhou/multiPhATE.git. 
...
NCBI Swissprot - download using blast+ command: `update_blastdb.pl swissprot` - Note: this downloads only the blast-formatted sequences; or download from Uniprot: `wget ftp://ftp.uniprot.org/pub/databases/uniprot/current_release/knowledgebase/complete/uniprot_sprot.fasta.gz` - unpack and format for blast. The `wget` method will download the fasta sequences, which will be needed if searching with phmmer or jackhmmer.
...
* Issue 2:  I am downloading databases on a remote server and my console keeps timing out or getting disconnected before dbPrep_getDBs.py finishes a download.  Solution: The script can be modified as a workaround for this problem. Edit the dbPrep_getDBs.py file as follows: set INTERACTIVE to False, and set REMOTE to True and VERBOSE to True (note: these words are case sensitive). Running dbPrep_getDBs.py in REMOTE mode will require that you pre-set the databases you want downloaded. Scroll down to the comment that says, "Pre-set download instructions; skip user input", and set the databases you want to True.
* Issue 3:  I'm getting an error message that looks something like this:  "Error while running gene calling-genecaller_1_geneCall has zero length". This probably means that a gene caller is failing. This might happen due to the way some gene callers handle fasta headers. It is always best to remove all non-alphanumeric characters from your fasta header strings. Run your genome fasta file through script cleanHeaders.py (in the Utility/ folder), capturing the output in a new file name, then adjust the genome fasta name in your config file, and try running multiPhATE2 again.
* Issue 4:  I'm running a phage genome through multiPhATE2, and the code is reporting a WARNING message when I select any gene caller (or subset) other than Phanotate. Solution: Ignore the warning. It's only telling you that you are running a phage genome through the code, but have selected a gene caller (or subset) other than the only phage-tailored gene caller, which is Phanotate.
* Issue 5:  In running blast (with fewer than 5 hits), I see the following warning message:  "Examining 5 or more matches is recommended". This message is generated by recent versions of blast+. You can ignore this message, or simply set the number of matches to 5 or more in your config file.
* Issue 6:  The default settings in the CompareGeneProfiles module are too stringent for my genome set. Solution:  Use checkpointing (see above) to re-run the CGP analysis. You may in your configuration file (multiphate.config) set the cgp_identity cutoff value. Re-run any number of times, until you are satisfied with the results of the Genomics module.
* Issue 7:  Some of the blast processes in the CompareGeneProfiles (CGP) module fail, yielding empty blast .out files in the Results_nnnnnn_nnnnnn directories. Solution: Turn off parallel processing in the CompareGeneProfiles module (ie, set cgp_threads='0' in your configuration file). We have discovered an issue with a third-party code, makeblastdb. CompareGeneProfiles runs makeblastdb for each genome's gene and protein fasta files (in each genome subdirectory). When CompareGeneProfiles is run in parallel (using one process for each genomeXgenome comparison), makeblastdb intermittently fails to produce one or more of the database files, typically the index file (.nin or .pin), or other supporting files (e.g., .nsq, .psq, .phr). You can check whether these files were created by listing the contents of a given genome's PhATE output ($ ls -l PipelineOutput/myGenomeSubdir/). For each myGenome_cgp_gene.fnt and myGenome_cgp_protein.faa file there should be 7 blast databases files (eg, .pdb, .phr, .pin, .pot, .psq, .ptf, .pto). If any one of these files is missing, then blast+ will fail, and the CGP_RESULTS/Results_nnnnnn_nnnnnn/ directories will show some blast output files with 0 content. An error message will be generated by blast, depending on which missing file is encountered. For example, you might see an error message stating that no index file was found, or blast might throw a C++ null-pointer exception. We are looking into how to manage the use of makeblastdb when using parallel processing in multiPhATE2. For now, we recommend setting cgp_threads='0' if you encounter this problem. Another workaround entails correcting makeblastdb's incorrect output by running makeblastdb by hand within the genome's PhATE output subdirectory for each affected fasta file, then re-running multiPhATE2 with the cgp_checkpoint set to 'true' in your configuration file. 
...
2) The developers have detected an issue with makeblastdb. When running the code in parallel, makeblastdb occasionally fails to produce all of the necessary blast database files, which ultimately causes blast to fail. More details can be found in a [TROUBLESHOOTING](#troubleshooting) issue above (see #7). 
