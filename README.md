...
2) The developers have detected an issue with makeblastdb. When running the code in parallel, makeblastdb occasionally fails to produce all of the necessary blast database files, which ultimately causes blast to fail. More details can be found in a [TROUBLESHOOTING](#troubleshooting) issue above (see #8). 
